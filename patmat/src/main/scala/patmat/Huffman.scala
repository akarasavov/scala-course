...
object Huffman {
  ...
  /**
    ...
    * }
    */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(k => k).map(k => (k._1, k._2.size)).toList
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    val combined: Fork = makeCodeTree(trees.head, trees(1))
    List(combined) ::: trees.drop(2)
  }
  ...
  def until(singleton: List[CodeTree] => Boolean, combine: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    val merged: List[CodeTree] = combine.apply(trees)
    if (singleton.apply(merged)) {
      merged
    }
    else {
      until(singleton, combine)(merged)
    }
  ...
  /**
    ...
    */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.find(e => e._1 == char).map(k => k._2).get
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encode(codeTable: CodeTable, text: List[Char]): List[Bit] = {
      text match {
        case (Nil) => List()
        case (head :: tail) => codeBits(codeTable)(head) ::: encode(codeTable, tail)
      }
    }
